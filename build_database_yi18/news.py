...
def main():
    ...
    while True:
        ...
        try:
            file_name = '%s/news/%d.txt' % (home_path, req_id)
            ...
            sql = 'INSERT INTO news(id, title, tag, content) VALUES(%s, %s, %s, %s)'
            cursor.execute(sql, (req_id, title, tag, content))
            db.commit()
            logging.info('Building DATABASE `yi18`, TABLE `news`, ' + file_name)
