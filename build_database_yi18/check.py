...
def main():
    ...
    while True:
        ...
        try:
            file_name = '%s/check/%d.txt' % (home_path, req_id)
            ...
            sql = 'INSERT INTO checks(id, name, menu, content) VALUES(%s, %s, %s, %s)'
            cursor.execute(sql, (req_id, name, menu, content))
            db.commit()
            logging.info('Building DATABASE `yi18`, TABLE `checks`, ' + file_name)
