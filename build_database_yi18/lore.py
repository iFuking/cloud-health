...
def main():
    ...
    while True:
        ...
        try:
            file_name = '%s/lore/%d.txt' % (home_path, req_id)
            ...
            sql = 'INSERT INTO lore(id, title, classname, content) VALUES(%s, %s, %s, %s)'
            cursor.execute(sql, (req_id, title, class_name, content))
            db.commit()
            logging.info('Building DATABASE `yi18`, TABLE `lore`, ' + file_name)
