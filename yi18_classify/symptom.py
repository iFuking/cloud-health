...
read_log_path = '/home/dick/filter/symptom/0_log.txt'
read_log = open(read_log_path, 'r')
write_log_path = '/home/dick/classify/bp/symptom/0_log.txt'
...
for line in read_log.readlines():
    ...
    for i in range(0, len(item)):
        ...
        if i % 2 and item[i].find(KEY_WORD) is not -1:
            req_id = item[0]
            # print type(req_id)
            read_path = '/home/dick/yi18/symptom/%s.txt' % req_id
            f = open(read_path, 'r')
            content = f.read()
            # print type(content)
            f.close()

            write_path = '/home/dick/classify/bp/symptom/%s.txt' % req_id
            f = open(write_path, 'w')
            f.write(content)
            f.close()
            # print type(line)
